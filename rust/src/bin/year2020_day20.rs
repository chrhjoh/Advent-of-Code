...
fn exercise1(data: &str) -> i64 {
    ...
    return image.get(&(min_x, min_y)).unwrap().0 as i64
        * image.get(&(min_x, max_y)).unwrap().0 as i64
        * image.get(&(max_x, min_y)).unwrap().0 as i64
        * image.get(&(max_x, max_y)).unwrap().0 as i64;
}
